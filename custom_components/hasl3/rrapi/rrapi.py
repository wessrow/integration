...
from .exceptions import RRAPI_Error, RRAPI_HTTP_Error, RRAPI_API_Error
from .const import (
    __version__,
    BASE_URL,
    STOP_LOOKUP_URL,
...
class rrapi_sl(rrapi):
    ...
    async def request(self, searchstring):
        ...
        data = await self._get(
            STOP_LOOKUP_URL.format(BASE_URL, searchstring, self._api_token),
            "Location Lookup",
        )
        result = []
        ...
        for stopOrLocation in data["stopLocationOrCoordLocation"]:
            place = (
                stopOrLocation["CoordLocation"]
                if "CoordLocation" in stopOrLocation
                else stopOrLocation["StopLocation"]
            )
            entry = {}
...
class rrapi_rrr(rrapi):
    ...
    async def request(self, origin, destination):
        ...
        return await self._get(
            ROUTE_PLANNER_URL.format(BASE_URL, origin, destination, self._api_token),
            "Route Planner",
        )
...
class rrapi_rrd(rrapi):
    ...
    async def request(self, id):
        ...
        return await self._get(
            DEPARTURE_BOARD_URL.format(BASE_URL, id, self._api_token), "Departure Board"
        )


class rrapi_rra(rrapi):
    def __init__(self, hass: HomeAssistant, api_token, id, timeout=None):
        super().__init__(hass, timeout)
        self._api_token = api_token
    ...
    async def request(self, id):
        ...
        return await self._get(
            ARRIVAL_BOARD_URL.format(BASE_URL, id, self._api_token), "Arrivals Board"
        )
