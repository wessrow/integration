...
class HASLInstances(object):
    ...
    def add(self, id, updater):
        self.instances[id] = {"subscriber": updater}
    ...
    def remove(self, id):
        try:
            ...
            logger.debug(f"Error occurred while deregistering listener {str(e)}")
...
class HaslWorker(object):
        ...
        if sensor is not None and not sensor == "":
            try:
                ...
                if sensor_state.state is state:
                    logger.debug(
                        "[check_sensor_state] Completed will return TRUE/ENABLED"
                    )
                    return True
                else:
                    logger.debug(
                        "[check_sensor_state] Completed will return FALSE/DISABLED"
                    )
                    return False
            except:
                logger.debug(
                    "[check_sensor_state] An error occurred, default will be returned"
                )
                return default
        else:
            logger.debug(
                "[check_sensor_state] No sensor specified, will return default"
            )
            return default
    ...
    async def assert_rp3(self, key, source, destination):
        ...
        if key not in self.data.rp3keys:
            ...
            self.data.rp3keys[key] = {"api_key": key, "trips": ""}
        else:
            ...
            for tripname in "|".join(set(rp3data["trips"].split("|"))).split("|"):
                ...
                try:
                    ...
                    apidata = await api.request(
                        srcLocID, dstLocID, srcLocLat, srcLocLng, dstLocLat, dstLocLng
                    )
                    newdata["trips"] = []
                    ...
                    lastLegLastTrip = next(
                        (
                            x
                            for x in reversed(newdata["trips"][0]["legs"])
                            if x["category"] != "WALK"
                        ),
                        [],
                    )
                    newdata["transfers"] = (
                        sum(
                            p["category"] != "WALK" for p in newdata["trips"][0]["legs"]
                        )
                        - 1
                        or 0
                    )
                    ...
                    newdata["to"] = (
                        newdata["trips"][0]["legs"][
                            len(newdata["trips"][0]["legs"]) - 1
                        ]["to"]
                        or ""
                    )
                    newdata["origin"] = {}
                    ...
                    newdata["origin"]["direction"] = (
                        firstLegFirstTrip["direction"] or ""
                    )
                    ...
                    newdata["destination"]["direction"] = (
                        lastLegLastTrip["direction"] or ""
                    )
                    newdata["destination"]["category"] = (
                        lastLegLastTrip["category"] or ""
                    )
    ...
    async def assert_si2(self, key, datakey, listkey, listvalue):
        ...
        if key not in self.data.si2keys:
            ...
            self.data.si2keys[key] = {"api_key": key, "stops": "", "lines": ""}
        else:
        ...
        if self.data.si2keys[key][listkey] == "":
            ...
            self.data.si2keys[key][listkey] = (
                f"{self.data.si2keys[key][listkey]},{listvalue}"
            )
        ...
        for si2key in list(self.data.si2keys):
            ...
            for stop in ",".join(set(si2data["stops"].split(","))).split(","):
                ...
                try:
                    ...
                    newdata["api_error"] = str(e)
                    logger.debug(
                        f"[process_si2] An error occurred during processing of stop {stop}"
                    )
            ...
            for line in ",".join(set(si2data["lines"].split(","))).split(","):
                ...
                try:
                    ...
                    newdata["api_error"] = str(e)
                    logger.debug(
                        f"[process_si2] An error occurred during processing of line {line}"
                    )
    ...
    async def assert_ri4(self, key, stop):
        ...
        if key not in self.data.ri4keys:
            ...
            self.data.ri4keys[key] = {"api_key": key, "stops": stopkey}
        else:
            logger.debug("[assert_ri4] Adding stop to existing key")
            self.data.ri4keys[key]["stops"] = (
                f"{self.data.ri4keys[key]['stops']},{stopkey}"
            )
    ...
    async def assert_rrd(self, key, stop):
        ...
        if key not in self.data.rrkeys:
            ...
            self.data.rrkeys[key] = {"api_key": key}
                ...
                try:
                    ...
                    for idx, value in enumerate(departuredata):
                        ...
                        if "rtDate" in value and "rtTime" in value:
                            ...
                            expected = datetime.strptime(
                                f'{value["date"]} {value["time"]}', "%Y-%m-%d %H:%M:%S"
                            )
                        ...
                        departures.append(
                            {
                                ...
                                "icon": iconswitcher.get(
                                    value["ProductAtStop"]["catOut"], "mdi:train-car"
                                ),
                            }
                        )
        ...
        for rrkey in list(self.data.rrkeys):
            ...
            for stop in ",".join(set(rrdata["arrs"].split(","))).split(","):
                ...
                try:
                    ...
                    newdata["data"] = sorted(arrivals, key=lambda k: k["time"])
                    newdata["attribution"] = "Samtrafiken Resrobot"
                    newdata["last_updated"] = now().strftime("%Y-%m-%d %H:%M:%S")
                    newdata["api_result"] = "Success"
        ...
        for tl2key in list(self.data.tl2):
            ...
            try:
                ...
                for response in apidata:
                    statustype = (
                        "ferry" if response["Type"] == "fer" else response["Type"]
                    )
